...
# Same list used in create_eval_questions_retrieval.py. A pair here
# means the clue would restate the target concept rather than give
# useful, distinct context.
BLOCKED_TARGET_CLUE_PAIRS = {
    ...
    (
        "History of autologous bone marrow transplant (situation)",
        "Awaiting transplantation of bone marrow (situation)",
    ),
}
...
def find_safe_clues(target_fact, encounter, all_field_counts):
    ...
    nowhere else in this patient's encounters, excluding any that
    would restate the target concept.
    """
    clues = []
    seen = set()
    target_core = strip_tag(target_fact).lower()
...
def main():
    ...
    for patient_id, encounters in patient_items:
        ...
        # Keep only facts that are the ONLY value in their field for
        # that specific encounter, so the question has exactly one
        # valid answer (an encounter with three procedures can't be
        # asked about with "What procedure...").
        singular_targets = []
        for fact, encounter_id in unique_targets:
            field = fact_to_field[fact]
            field_values = data[encounter_id].get(field, []) or []
            if len(field_values) == 1:
                singular_targets.append((fact, encounter_id))

        unique_targets = singular_targets
    ...
    print("Each question's field is the only value of that type in its encounter,")
    print("so every question has exactly one valid answer.")
